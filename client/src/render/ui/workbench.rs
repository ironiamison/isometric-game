...
impl Renderer {
    pub(crate) fn render_workbench(
        ...
        for (idx, label) in tab_labels.iter().enumerate() {
            ...
            if is_active {
                draw_line(
                    ...
                    2.0, Color::new(0.70, 0.50, 0.25, 1.0),
                );
            } else {
                draw_line(
                    tx + 4.0 * s, tab_y + tab_h - 1.0,
        ...
        if state.ui_state.crafting_in_progress {
            self.render_workbench_progress(state, hovered, layout, content_x, content_y, content_w, total_content_h);
        } else {
            ...
            self.render_workbench_recipe_list(state, hovered, layout, content_x, content_y, content_w, recipe_list_h);
            ...
            self.render_workbench_crafting_detail(state, hovered, layout, content_x, detail_y, content_w, detail_h);
        }
        ...
        if state.ui_state.crafting_in_progress {
            ...
            self.draw_text_sharp("[Tab] Tab", header_x + 230.0 * s, footer_y + footer_h * 0.67, 16.0, TEXT_DIM);
            self.draw_text_sharp("[Enter] Craft", header_x + 330.0 * s, footer_y + footer_h * 0.67, 16.0, TEXT_DIM);
        }
    }

    /// Render the skill info bar: survivalist level, XP bar, recipe count
    fn render_workbench_skill_bar(
        &self,
        state: &GameState,
        x: f32,
        y: f32,
        ...
        if fill_w > 0.0 {
            draw_rectangle(bar_x + 2.0, bar_y + 2.0, fill_w, bar_h - 4.0, WORKBENCH_PROGRESS_DARK);
            draw_rectangle(bar_x + 2.0, bar_y + 2.0, fill_w, (bar_h - 4.0) / 2.0, WORKBENCH_PROGRESS_MID);
        }
        ...
        let all_workbench: Vec<_> = state
            ...
            .filter(|r| r.station.as_deref() == Some("workbench"))
            .collect();
        let unlocked = all_workbench.iter().filter(|r| {
    ...
    fn render_workbench_recipe_list(
        ...
        let scroll_offset = state.ui_state.workbench_scroll_offset.clamp(0.0, max_scroll);
        ...
        for recipe in &workbench_recipes {
            ...
            if is_selected {
                ...
                draw_rectangle(content_x + 2.0, y + 4.0, 3.0, row_height - 8.0, Color::new(0.70, 0.50, 0.25, 1.0));
            }
    ...
    fn render_workbench_crafting_detail(
        ...
        let is_craft_hovered = can_craft && matches!(hovered, Some(UiElementId::WorkbenchCraftButton));
        let (btn_bg, btn_border) = if !can_craft {
            (Color::new(0.12, 0.08, 0.06, 1.0), SLOT_BORDER)
        } else if is_craft_hovered {
            (Color::new(0.50, 0.35, 0.15, 1.0), Color::new(0.70, 0.50, 0.25, 1.0))
        } else {
            (Color::new(0.40, 0.28, 0.12, 1.0), Color::new(0.60, 0.42, 0.20, 1.0))
        };

        draw_rectangle(craft_btn_x, craft_btn_y, craft_btn_w, craft_btn_h, btn_border);
        draw_rectangle(
            craft_btn_x + 1.0,
            craft_btn_y + 1.0,
            craft_btn_w - 2.0,
        ...
        let craft_text = if can_craft { "[ CRAFT ]" } else { "Can't Craft" };
    ...
    fn render_workbench_progress(
        ...
        if fill_width > 0.0 {
            ...
            draw_rectangle(fill_x, fill_y, fill_width, fill_h / 2.0, WORKBENCH_PROGRESS_MID);
            draw_line(fill_x, fill_y, fill_x + fill_width, fill_y, 1.0, WORKBENCH_PROGRESS_LIGHT);
        }
