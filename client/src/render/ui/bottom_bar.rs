...
impl Renderer {
    ...
    pub(crate) fn render_exp_bar(&self, state: &GameState) {
        ...
        // Get player exp data
        let (current_exp, exp_to_next) = if let Some(player) = state.get_local_player() {
        ...
        let fill_pct = if exp_to_next > 0 {
            (current_exp as f32 / exp_to_next as f32).clamp(0.0, 1.0)
        } else {
            0.0
        };

        // Draw fill bar (simple green, with small top margin for border)
        let fill_width = bar_width * fill_pct;
        if fill_width > 0.0 {
        ...
        let exp_text = format!("EXP: {} / {}", current_exp, exp_to_next);
    ...
    pub(crate) fn render_menu_buttons(&self, state: &GameState, hovered: &Option<UiElementId>, layout: &mut UiLayout) {
        ...
        // 5 buttons: Character, Inventory, Social, Map, Settings
        let num_buttons = 5;
        ...
        let buttons = [
            (UiElementId::MenuButtonCharacter, "C", state.ui_state.character_open),
            (UiElementId::MenuButtonInventory, "I", state.ui_state.inventory_open),
            (UiElementId::MenuButtonSocial, "S", state.ui_state.social_open),
            (UiElementId::MenuButtonMap, "M", state.ui_state.map_open),
            (UiElementId::MenuButtonSettings, "O", state.ui_state.escape_menu_open),
        ];

        for (i, (element_id, label, is_active)) in buttons.iter().enumerate() {
            ...
            // Draw button
            self.draw_menu_button(x, y, MENU_BUTTON_SIZE, label, is_hovered, *is_active);
        }
    }

    /// Draw a single menu button
    fn draw_menu_button(&self, x: f32, y: f32, size: f32, label: &str, is_hovered: bool, is_active: bool) {
        ...
        // Label centered (placeholder until icons are added)
