...
impl Renderer {
    pub(crate) fn render_collection_log(
        ...
        draw_line(header_x, header_y + header_h, header_x + header_w, header_y + header_h, 1.0, HEADER_BORDER);

        // Title
        let title = "Collection Log";
        let title_w = self.measure_text_sharp(title, 16.0).width;
        self.draw_text_sharp(title, (header_x + (header_w - title_w) / 2.0).floor(), (header_y + 17.0 * s).floor(), 16.0, TEXT_TITLE);
        ...
        let close_color = if close_hovered { Color::new(1.0, 0.3, 0.3, 1.0) } else { TEXT_DIM };
        self.draw_text_sharp(close_text, close_x, (header_y + 17.0 * s).floor(), 16.0, close_color);
        layout.add(UiElementId::CollectionLogClose, Rect::new(close_x - 4.0 * s, header_y, close_w + 8.0 * s, header_h));
        ...
        // Vertical divider
        draw_line(grid_x, content_y, grid_x, content_y + content_h, 1.0, HEADER_BORDER);

        // ===== LEFT SIDEBAR =====
        self.render_collection_sidebar(state, hovered, layout, content_x, content_y, sidebar_w, content_h);
    ...
    fn render_collection_sidebar(
        ...
        for (cat_idx, (key, label)) in categories.iter().enumerate() {
            ...
            // Draw category row
            if cat_hovered {
                draw_rectangle(x + 2.0, cur_y - 2.0, w - 4.0, line_height + 4.0, SLOT_HOVER_BG);
            }

            let arrow = if is_expanded { "v" } else { ">" };
            let all_done = got == total && total > 0;
            let cat_color = if all_done { Color::new(0.0, 0.8, 0.0, 1.0) } else if cat_hovered { TEXT_TITLE } else { TEXT_NORMAL };
            ...
            if is_expanded {
                ...
                for (name, sub_total, sub_got) in &sorted_subcats {
                    let is_selected = state.ui_state.collection_log_selected_subcategory.as_deref() == Some(*name);
                    let sub_hovered = matches!(hovered, Some(UiElementId::CollectionLogSubcategoryEntry(idx)) if *idx == global_subcat_idx);

                    if is_selected {
                        draw_rectangle(x + 2.0, cur_y - 2.0, w - 4.0, line_height + 4.0, SLOT_HOVER_BORDER);
                        draw_rectangle(x + 3.0, cur_y - 1.0, w - 6.0, line_height + 2.0, SLOT_HOVER_BG);
                    } else if sub_hovered {
                        draw_rectangle(x + 2.0, cur_y - 2.0, w - 4.0, line_height + 4.0, SLOT_HOVER_BG);
                    }
                    ...
                    layout.add(UiElementId::CollectionLogSubcategoryEntry(global_subcat_idx), sub_rect);
    ...
    fn render_collection_grid(
        ...
        for (i, item_id) in items.iter().enumerate() {
            ...
            if item_hovered {
                ...
                draw_rectangle(slot_x + slot_size - 1.0, slot_y, 1.0, slot_size, SLOT_HOVER_BORDER);
            }

            // Register grid item for hover
            layout.add(UiElementId::CollectionLogGridItem(i), Rect::new(slot_x, slot_y, slot_size, slot_size));
        }
