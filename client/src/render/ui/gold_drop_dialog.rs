...
use super::common::*;
use crate::game::{GoldDropDialog, StallPriceDialog};
...
impl Renderer {
    ...
    pub(crate) fn render_gold_drop_dialog(
        &self,
    ...
    /// Render the stall price input dialog
    pub(crate) fn render_stall_price_dialog(
        &self,
        dialog: &StallPriceDialog,
        hovered: &Option<UiElementId>,
        layout: &mut UiLayout,
    ) {
        let (sw, sh) = virtual_screen_size();
        ...
        // Item name label
        let label = format!("Price per unit ({})", &dialog.item_id);
        self.draw_text_sharp(&label, content_x, content_y + 16.0, 16.0, TEXT_NORMAL);
        ...
        if dialog.input.is_empty() {
            self.draw_text_sharp("Enter price...", input_text_x, input_text_y, 16.0, TEXT_DIM);
        } else {
            self.draw_text_sharp(&dialog.input, input_text_x, input_text_y, 16.0, TEXT_NORMAL);
        }
        ...
        let confirm_x = content_x;
        let confirm_bounds = Rect::new(confirm_x, button_y, button_width, button_height);
        layout.add(UiElementId::StallPriceConfirm, confirm_bounds);

        let confirm_hovered = matches!(hovered, Some(UiElementId::StallPriceConfirm));
        let (confirm_bg, confirm_border) = if confirm_hovered {
            (Color::new(0.235, 0.204, 0.141, 1.0), FRAME_ACCENT)
        } else {
            (Color::new(0.157, 0.141, 0.110, 1.0), FRAME_MID)
        ...
        let cancel_x = content_x + button_width + 12.0;
        let cancel_bounds = Rect::new(cancel_x, button_y, button_width, button_height);
        layout.add(UiElementId::StallPriceCancel, cancel_bounds);
