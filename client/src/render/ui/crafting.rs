...
impl Renderer {
    pub(crate) fn render_crafting(
        ...
        let content_height =
            panel_height - FRAME_THICKNESS * 2.0 - HEADER_HEIGHT - FOOTER_HEIGHT - 12.0;
    ...
    fn render_recipes_tab(
        ...
        let list_y = if show_tabs {
            tab_y + tab_height + 12.0
        } else {
            content_y + 2.0
        };
        let list_height = if show_tabs {
            content_height - tab_height - 20.0
        } else {
            content_height - 10.0
        };
        ...
        for (_orig_idx, recipe, is_discovered) in &visible_recipes {
            ...
            if *is_discovered {
                ...
                if recipe.level_required > 1 {
                    ...
                    self.draw_text_sharp(
                        &level_text,
                        list_x + list_width - level_width - 12.0,
                        text_y,
                        16.0,
                        FRAME_MID,
                    );
        ...
        if let Some(recipe) = discovered_recipes.get(state.ui_state.crafting_selected_recipe) {
            ...
            if let Some(ref station) = recipe.station {
                ...
                self.draw_text_sharp(
                    &station_display,
                    detail_x + detail_width - station_w - 12.0,
                    name_y,
                    16.0,
                    TEXT_DIM,
                );
            ...
            for ingredient in &recipe.ingredients {
                ...
                self.draw_text_sharp(
                    &text,
                    detail_x + 20.0 + ing_icon_size + 6.0,
                    section_y + 2.0,
                    16.0,
                    color,
                );
        ...
        if let Some(ref recipe_id) = state.ui_state.crafting_recipe_id {
            if let Some(recipe) = state.recipe_definitions.iter().find(|r| &r.id == recipe_id) {
                ...
                self.draw_text_sharp(
                    &recipe.display_name,
                    name_x,
                    detail_y + 50.0 + progress_icon_size + 16.0,
                    16.0,
                    pulse_color,
                );
                ...
                if let Some(result) = recipe.results.first() {
                    ...
                    self.draw_text_sharp(
                        &result_text,
                        result_x,
                        detail_y + 50.0 + progress_icon_size + 36.0,
                        16.0,
                        TEXT_NORMAL,
                    );
                }
