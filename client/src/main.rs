...
fn run_game_frame(
    ...
    if game_state.debug_mode {
        ...
        // Ping stats
        if game_state.ping_stats.has_data() {
            let ps = &game_state.ping_stats;
            let ping_color = if ps.current_ms > 200.0 {
                Color::from_rgba(255, 100, 100, 255) // red for high ping
            } else if ps.current_ms > 120.0 {
                Color::from_rgba(255, 200, 100, 255) // yellow for moderate
            } else {
                Color::from_rgba(100, 255, 150, 255) // green for good
            };
        ...
        if let Some(player) = game_state.get_local_player() {
            ...
            renderer.draw_text_sharp(&format!("Pos: ({:.1}, {:.1})", player.x, player.y), 10.0, 100.0, 16.0, YELLOW);
            renderer.draw_text_sharp(&format!("Chunk: ({}, {})", chunk_x, chunk_y), 10.0, 120.0, 16.0, YELLOW);
            renderer.draw_text_sharp(&format!("NPCs: {}", game_state.npcs.len()), 10.0, 140.0, 16.0, WHITE);
            // Appearance debug info
            renderer.draw_text_sharp(&format!("Appearance: {} {} (F5/F6 to cycle)", player.gender, player.skin), 10.0, 160.0, 16.0, Color::from_rgba(150, 200, 255, 255));
        }
        ...
        renderer.draw_text_sharp("--- Frame Timing (ms) ---", 10.0, 190.0, 16.0, timing_color);
        renderer.draw_text_sharp(&format!("Network:  {:.2}", t.network_ms), 10.0, 210.0, 16.0, timing_color);
        ...
        renderer.draw_text_sharp(&format!("Render:   {:.2} (total)", t.render_total_ms), 10.0, 230.0, 16.0, timing_color);
        renderer.draw_text_sharp(&format!("  Ground:   {:.2}", t.render_ground_ms), 10.0, 250.0, 16.0, ground_color);
        renderer.draw_text_sharp(&format!("  Entities: {:.2}", t.render_entities_ms), 10.0, 270.0, 16.0, entities_color);
        renderer.draw_text_sharp(&format!("  Overhead: {:.2}", t.render_overhead_ms), 10.0, 290.0, 16.0, overhead_color);
        renderer.draw_text_sharp(&format!("  Effects:  {:.2}", t.render_effects_ms), 10.0, 310.0, 16.0, effects_color);
        renderer.draw_text_sharp(&format!("  UI:       {:.2}", t.render_ui_ms), 10.0, 330.0, 16.0, ui_color);

        renderer.draw_text_sharp(&format!("Update:   {:.2}", t.update_ms), 10.0, 350.0, 16.0, timing_color);
        renderer.draw_text_sharp(&format!("Total:    {:.2}", t.total_ms), 10.0, 370.0, 16.0, timing_color);
        renderer.draw_text_sharp(&format!("Entities: {} | Chunks: {}", t.entity_count, t.chunk_count), 10.0, 390.0, 16.0, timing_color);
        ...
        renderer.draw_text_sharp(&format!("Delta: {:.1}ms (range: {:.1}-{:.1}, var: {:.1})",
            t.delta_ms, t.delta_min_ms, t.delta_max_ms, delta_variance), 10.0, 410.0, 16.0, variance_color);
        ...
        renderer.draw_text_sharp(&format!("next_frame(): {:.1}ms (range: {:.1}-{:.1}, var: {:.1})",
            t.next_frame_ms, t.next_frame_min_ms, t.next_frame_max_ms, nf_variance), 10.0, 430.0, 16.0, nf_color);
        ...
        renderer.draw_text_sharp(&format!("Smoothing: {} [F7] (smoothed: {:.1}ms)",
            smooth_str, t.smoothed_delta * 1000.0), 10.0, 450.0, 16.0, timing_color);
    }
